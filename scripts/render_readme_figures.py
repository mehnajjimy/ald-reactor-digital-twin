...
# where the saved plot data, its provenance and the figures live
...
TEAL = "#168e96"
INK = "#20364b"
MUTED = "#526579"      # subtitles and tick labels
EDGE = "#c3ccd5"       # axis lines and phase dividers on white
DIVIDER = "#778fa4"    # phase dividers drawn over the heatmaps

# cycle timing in carrier residence times: A pulse, purge, B pulse, purge
A_END = 3
B_START = 8
B_END = 11
CYCLE_END = 16
PHASE_BOUNDARIES = (A_END, B_START, B_END)


def style():
    """shared matplotlib look for every readme figure."""
    plt.rcParams.update({"font.family": "DejaVu Sans", "font.size": 12,
        "axes.spines.top": False, "axes.spines.right": False,
        "axes.labelcolor": INK, "text.color": INK, "axes.edgecolor": EDGE,
...
def finish(fig, name, title, subtitle):
    """add the title and subtitle, save to docs/media and close."""
    fig.suptitle(title, x=.09, ha="left", fontsize=21, weight="bold", y=.97)
    fig.text(.09, .865, subtitle, fontsize=11, color=MUTED)
...
def cycle(data):
    """delivery timing, coverage and mass change over one ALD cycle."""
    fig, axes = plt.subplots(3, 1, figsize=(11, 7.5), sharex=True)
    fig.subplots_adjust(left=.14, right=.96, top=.84, bottom=.1, hspace=.28)
    t = data["time_tau"]

    # top: which precursor is on
    axes[0].step([0, A_END, A_END, CYCLE_END], [1, 1, 0, 0], color=BLUE, label="A inlet", lw=2.5)
    axes[0].step([0, B_START, B_START, B_END, B_END, CYCLE_END], [0, 0, 1, 1, 0, 0],
        color=TEAL, label="B inlet", lw=2.5)
    axes[0].set_ylabel("Delivery\n(on / off)")
    axes[0].set_ylim(-.1, 1.35)
    axes[0].set_yticks([0, 1])
    axes[0].legend(loc="upper right", ncol=2, frameon=False)

    # middle: coverage mean and spatial range
    ...
    for ax in axes:
        for boundary in PHASE_BOUNDARIES:
            ax.axvline(boundary, color=EDGE, ls=":", lw=1)
...
def comparison(data):
    """0D against spatial completion and turnover along the channel."""
    fig, axes = plt.subplots(1, 2, figsize=(11, 5.5))
    fig.subplots_adjust(left=.09, right=.97, top=.76, bottom=.15, wspace=.3)
    z = data["position_mm"]

    # left: completion in percent
...
def heatmap(data):
    """gas fraction and coverage over position and time."""
    fig, axes = plt.subplots(1, 2, figsize=(11, 5.5), sharey=True)
    fig.subplots_adjust(left=.09, right=.96, top=.74, bottom=.16, wspace=.35)
    panels = [("gas_a", "A gas fraction / inlet fraction"), ("theta", "A termination fraction")]
    for ax, (field, title) in zip(axes, panels):
        ...
        ax.set_xlim(0, CYCLE_END)
        for boundary in PHASE_BOUNDARIES:
            ax.axvline(boundary, color=DIVIDER, ls=":", lw=.8)
...
def main():
    """check the saved data against its provenance hash, then draw all three figures."""
